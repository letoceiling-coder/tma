    ...
    protected function handleComposerInstall(): array
    {
        try {
            ...
            if (empty($composerPath) || $composerPath === 'composer') {
                ...
                // Пробуем несколько способов обхода проблем с правами доступа
                $escapedPath = escapeshellarg($composerPath);
                
                // Способ 1: Используем php8.2 для выполнения composer файла напрямую
                ...
                // Если это не работает, можно попробовать через bash -c для обхода ACL
                // Но сначала пробуем простой способ
            }
